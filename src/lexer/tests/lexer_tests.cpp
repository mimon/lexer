...
SCENARIO("A language with regexs that contain capture sequences should be supported") {
  GIVEN("A language") {
    lexer::regex_vector tokens {
      regex("(\\d+)"),
      std::regex("[-+]?(\\d*[.])(\\d*)", std::regex::ECMAScript) // float
    };

    lexer::generic_lexer lexer(tokens);

    GIVEN("A string that contains a float literal") {
      string text("123.123");
      ...
      WHEN("Tokenizing the text") {
        ...
        THEN("The node should span over the entire regex match (and be split by capture sequences)") {
          CHECK(lexer.nodes[0].p == 0);
          CHECK(lexer.nodes[0].n == 7);
        }
      }
    }
  }
